...
p1 = [19.56338617791189, 257.1738063093089]
p2 = [273.23498070743557, 34.64400736176246]
p3 = [437.0039934810713, 75.08928053680133]
p4 = [470.0692529116021, 221.63433641494012]
p5 = [31.91578089170987, 215.59832182237633]
...
while not crashed:
	...
	for i in range(num_play):
		if neigh[i][1] == num_play-1:
			...
			##################    DIST**2/AREA ALGORITHM    ################
			k = 4*area/dist(points[neigh[i][1]],points[num_play-1])**2
			if i!=0 and i!=num_play-1:
				#d = dist(points[neigh[i][1]],points[neigh[i+1][1]])/(dist(points[neigh[i][1]],points[neigh[i-1][1]])+epsilon)
				nu = k*(points[neigh[i][1]][1]-points[num_play-1][1])+(points[neigh[i+1][1]][0]-points[neigh[i-1][1]][0])
				de = k*(points[neigh[i][1]][0]-points[num_play-1][0])-(points[neigh[i+1][1]][1]-points[neigh[i-1][1]][1])
				beta = atan2(nu,de)
				#peri2 = peri-dist(points[neigh[i][1]],points[neigh[i+1][1]])-dist(points[neigh[i][1]],points[neigh[i-1][1]])
				next = neigh[i+1][1]
				prev = neigh[i-1][1]
			elif i==num_play-1:
				nu = k*(points[neigh[i][1]][1]-points[num_play-1][1])+(points[neigh[0][1]][0]-points[neigh[i-1][1]][0])
				de = k*(points[neigh[i][1]][0]-points[num_play-1][0])-(points[neigh[0][1]][1]-points[neigh[i-1][1]][1])
				beta = atan2(nu,de)
				#peri2 = peri-dist(points[neigh[i][1]],points[neigh[0][1]])-dist(points[neigh[i][1]],points[neigh[i-1][1]])
				next = neigh[0][1]
				prev = neigh[i-1][1]
			elif i==0:
				nu = k*(points[neigh[i][1]][1]-points[num_play-1][1])+(points[neigh[i+1][1]][0]-points[neigh[num_play-1][1]][0])
				de = k*(points[neigh[i][1]][0]-points[num_play-1][0])-(points[neigh[i+1][1]][1]-points[neigh[num_play-1][1]][1])
				beta = atan2(nu,de)
